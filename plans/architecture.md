...
- **Service 2 - webhook-processor**: consumes events from the queue, looks up which customers
  subscribed to that event type (from `subscriptions.json`), and delivers the payload over HTTP
...
flowchart LR
  ...
  Subs["subscriptions.json"] -.->|"who wants which event"| Processor
...
sequenceDiagram
  ...
  Q-->>W: message batch
  W->>W: match event type to subscriptions.json
...
- **Config**: subscriptions load from a generated `configs/subscriptions.json` (see Section 5) via
  the standard library `encoding/json`, plus environment variables for everything else. Upgrade to
  `koanf` or `viper` only if config gets unwieldy.
...
hookwave/
  ...
  internal/
    ...
    config/          # load env + subscriptions.json
    subscriptions/   # subscription model + lookup by event type
    httpx/           # shared HTTP client/server helpers, middleware
    observability/   # OTel + slog setup (one init shared by all 3 services)
  configs/
    subscriptions.json  # generated by `cmd/subscriber` (GENERATE_SUBSCRIPTIONS=N), not hand-written
  deploy/

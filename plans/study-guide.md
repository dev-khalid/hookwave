...
- (must) `encoding/json`: struct tags, `Marshal`/`Unmarshal`, handling unknown fields - this is also how
  you load `subscriptions.json` (generated by `cmd/subscriber`, not a hand-written YAML file).
...
`subscriptions.json` is read-only after load (read-only sharing is safe). You will, however, hit the exact
...
- Hot-reloading `subscriptions.json` at runtime (an enhancement) -> classic `sync.RWMutex` (many readers, rare writer).
